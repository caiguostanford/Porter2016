...
=======
Key analysis is the 3rd analysis in the previous section.
>>>>>>> origin/master
...
summary(lm(Helpful~LIB, ds)) #simple effects of LIB
summary(lm(Helpful~LIB*Party, ds)) #interaction between LIB and target's political group
summary(lm(Rude~LIB, ds)) #simple effects of LIB
...
=======
#social inference graph
ggplot(ds, aes(x=Party, y=Communicator, fill=LIB))+
  geom_bar(position="dodge", stat = "identity") +
    geom_errorbar(aes(ymin=Communicator-sem(Communicator), ymax=Communicator+sem(Communicator)), width=.25,position=position_dodge(.9)) + facet_grid(. ~Political) #faceted by participants' own group membership

#friendship graph
ggplot(ds, aes(x=Party, y=Friend, fill=LIB))+
  geom_bar(position="dodge", stat = "identity") +
    geom_errorbar(aes(ymin=Friend-sem(Friend), ymax=Communicator+sem(Friend)), width=.25,position=position_dodge(.9)) + facet_grid(. ~Political) #faceted by participants' own group membership
>>>>>>> origin/master
```
